struct SegBeat {
    ...
    SegBeat(int _n) : n(_n) {
        max_v.resize(4*_n+4,0);smax_v.resize(4*_n+4,-1e18);max_c.resize(4*_n+4,1);
        min_v.resize(4*_n+4,0);smin_v.resize(4*_n+4,1e18);min_c.resize(4*_n+4,1);
    ...
    void add_val(int a, int b, ll x) {_add_val(x, a-1, b, 0, 0, n0);}
    void update_val(int a, int b, ll x) {_update_val(x, a-1, b, 0, 0, n0);}
    ll query_max(int a, int b) {return _query_max(a-1, b, 0, 0, n0);}
    ll query_min(int a, int b) {return _query_min(a-1, b, 0, 0, n0);}
    ll query_sum(int a, int b) {return _query_sum(a-1, b, 0, 0, n0);}
};
